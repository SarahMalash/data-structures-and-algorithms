...
const countNumberOfElements = (arr) => {
  let array1Length = 1;
  let counterArray = arr.reduce((length, value) => {
    array1Length++;
    return counterArray;
  });
  return array1Length;
...
const reversedString = (str) => {
  ...
  let Array2 = Array1.reduce((acc, value) => {
    acc.unshift(value);
    return acc;
  },[]);
  return Array2.join('')
...
const countNumberOfChildren = (arr) => {
  let Count = arr.reduce((acount, character) => {

    if (character.property('children')) {
      acount = acount + character.children.length;
    }
    return acount;
  }, 0);
  return Count;
