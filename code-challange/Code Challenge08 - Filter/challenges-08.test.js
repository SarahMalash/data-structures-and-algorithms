...
const oddValues = (arr) => {
  let oddNumbers = arr.filter(value => {
    return value % 2 !== 0;
  })
  return oddNumbers;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  ...
  forbiddenValues.forEach(Value => {
    ...
    Array1 = Array1.filter(candidate => {
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  let Array1 = arr.filter(Obj1 => {
    return Obj1.baseStat > minBaseStat;
  });
  return Array1;
};
...
const getStatName = (arr, minBaseStat) => {
  ...
  let state = Array1.map(Obj1 => Obj1.stat.name);
  return state;
};
